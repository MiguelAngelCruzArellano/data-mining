...
Escribe la fecha actual (**NOTE-01**): 03/06/2000
...
```{r}
NOMBRE_COMPLETO = "Miguel Angel Cruz Arellano"
DIGITOS_MATRICULA = 01748356
```
...
- Densidad de la matriz (**NOTE-10**): 0.566667
...
```{r}
try(
  plot(smalltx, main = paste0("Elaborado por: Miguel Angel Cruz Arellano"," (", 01748356, ")" ))  
)
...
```{r}
try(
  itemFrequencyPlot(smalltx, topN=10, main = paste0("Elaborado por: Miguel Angel Cruz Arellano" , " (", 01748356, ")" ))
)
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): Que este elemento no está relacionado con otro en una transacción.
...
Porque para generar una lista de 46 reglas, smallrules está parametrizando los valores tanto para support, confidence, coverage y lift, por lo que dentro de support y confidence el valor mínimo dado está con 0.2 y 0.8 respectivamente, por lo que podría entrar en support pero no en confidence, así que no aparece en la lista de reglas.
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Su función registar datos relacionados a distinguir a cada una de las sesiones que el usuario maneja y conocer la interacción que tiene con la plataforma. Esto funciona como un modelado para reconocer el comportamiento y comenzar a generar recomendaciones.
- ¿Cuál es el propósito de `track id`? (**NOTE-24**):Representa una interacción con cada una de las canciones.
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Sí, ya que todas estas columnas pueden generar reglas que definan patrones o secuencias como el salto de canciones, poner pausa o no antes del cambio de canción, adelantar o retroceder y así también empezar a comprender gustos y generar sugerencias.
...
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): Que en promedio los usuarios tienden a estar 14.62 veces en la aplicación.
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**): Los valores por default que adquieren support y confidence hace que no se logren generar reglas que cumplan esos parámetros.
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? Que se generarían bastantes reglas y esto a la vez no daría información relevante para el analista, ya que no sería tan puntual.
...
```{r}
try(
  plot(rules, main=paste0("Elaborado por: Miguel Angel Cruz Arellano" , " (", 01748356, ")" ))
)
...
- Provee el enlace al video aquí: **NOTE-30**: https://www.youtube.com/watch?v=7yPvcVbZrX8
